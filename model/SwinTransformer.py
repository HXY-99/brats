...
from ShiftedPatch import ShiftedPatchTokenization
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        if not is_spt:
            ...
            self.img_resolution = (pretrain_img_size//patch_size, pretrain_img_size//patch_size)
...
class Upsample_block_last(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(Upsample_block_last, self).__init__()
        self.transconv1 = nn.ConvTranspose2d(in_channels, out_channels, 4, padding=1, stride=2)
        self.conv1 = nn.Conv2d(6, out_channels, 3, padding=1)
...
class transformer_model(nn.Module):
    def __init__(self):
        ...
        self.up4 = Upsample_block(128, 3)

    def forward(self, x):
        x1, x2, x3, x4 = self.transformer(x)
        y = F.dropout2d(F.relu(self.bn(self.conv(x4))))
        y = self.up1(y, x3)
        y = self.up2(y, x2)
        y = self.up3(y, x1)
        y = self.up4(y, x)
...
if __name__ == "__main__":
    ...
    # print(model_dict)
    pretrained_dict = torch.load(r'E:\PythonCode\program\pretrained\upernet_swin_base_patch4_window7_512x512.pth')
    # # pretrained_dict = torch.load('../models/Swintranformer/model.pth')
