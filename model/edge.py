...
class ESAM(nn.Module):
    ...
    def forward(self, x):
        y = run_sobel(self.sobel_x2, self.sobel_y2, x)
        y = F.relu(self.bn(y))
        y = self.conv1(y)
        y = x + y
        y = self.conv2(y)
...
class Edgenet(nn.Module):
    ...
    def forward(self, x, feature=False):
        ...
        out2 = self.up2(y3)
        out2 = F.interpolate(out2, (120, 120), mode='bilinear', align_corners=True)

        out3 = self.up3(y4)
        out3 = F.interpolate(out3, (120, 120), mode='bilinear', align_corners=True)

        out4 = self.up4(y5)
        out4 = F.interpolate(out4, (120, 120), mode='bilinear', align_corners=True)
        ...
        out = F.interpolate(out, (240, 240), mode='bilinear', align_corners=True)
